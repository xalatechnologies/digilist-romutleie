...
export const Button: React.FC<ComponentProps & { 
  ...
  const variants = {
    primary: "bg-primary text-primary-foreground hover:bg-primary/90 shadow-token-sm transition-token-base",
    secondary: "bg-secondary text-secondary-foreground hover:bg-secondary/80 shadow-token-sm transition-token-base",
    ...
    ghost: "text-foreground hover:bg-muted hover:text-foreground transition-token-base",
    destructive: "bg-destructive text-destructive-foreground hover:bg-destructive/90 shadow-token-sm transition-token-base",
    success: "bg-success text-success-foreground hover:bg-success/90 shadow-token-sm transition-token-base",
    warning: "bg-warning text-warning-foreground hover:bg-warning/90 shadow-token-sm transition-token-base",
    info: "bg-info text-info-foreground hover:bg-info/90 shadow-token-sm transition-token-base"
  };
  const sizes = {
...
export const Card: React.FC<ComponentProps & { onClick?: () => void }> = ({ children, className = '', onClick }) => (
  <div 
    className={`card-base transition-token-base ${className}`}
    onClick={onClick}
  >
    {children}
  </div>
...
export const Input: React.FC<React.InputHTMLAttributes<HTMLInputElement>> = (props) => (
  <input 
    {...props}
    className={`input-base transition-token-base ${props.className || ''}`}
  />
);
...
export const Modal: React.FC<{
  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center p-4" style={{ zIndex: 'var(--z-modal)' }}>
      ...
      />
      <div className="relative w-full max-w-2xl bg-background rounded-token-xl shadow-token-2xl overflow-hidden animate-in zoom-in-95 fade-in transition-token-base" style={{ zIndex: 'var(--z-modal)' }}>
        <div className="flex items-center justify-between p-6 border-b bg-muted/30">
          <Text size="lg" weight="bold">{title}</Text>
          <Button variant="ghost" size="icon" onClick={onClose} className="rounded-token-full">
            <X size={24} />
          </Button>
        </div>
      ...
      <div className="flex-1 flex flex-col w-full">
        <header className="flex h-14 sm:h-16 items-center justify-between border-b border-border bg-card/80 backdrop-blur-md sticky top-0 px-3 sm:px-4 md:px-6 lg:px-8 w-full shadow-token-sm" style={{ zIndex: 'var(--z-sticky)' }}>
          <div className="flex items-center gap-2 sm:gap-4 min-w-0 flex-1">
            ...
            {onSidebarToggle && (
              <button
                ...
                className="hidden lg:flex items-center justify-center w-9 h-9 rounded-token-lg hover:bg-muted transition-all duration-200 text-muted-foreground hover:text-foreground"
          ...
          <div className="flex items-center gap-2 sm:gap-4 md:gap-8 shrink-0">
            <Button variant="ghost" size="icon" className="relative h-10 w-10 sm:h-11 sm:w-11 touch-manipulation">
            ...
            <div className="hidden sm:flex items-center gap-2 md:gap-3 border-l border-border pl-3 md:pl-6 ml-1 md:ml-2">
              <div className="flex items-center gap-2 md:gap-3">
                {/* User Avatar */}
                <Avatar src={user?.avatar || ""} fallback={userRole || "U"} className="border-2 border-primary/20 shadow-token-sm w-8 h-8 md:w-10 md:h-10" />
